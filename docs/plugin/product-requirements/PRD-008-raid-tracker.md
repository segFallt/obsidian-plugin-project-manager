...
**Optional `sort` configuration:** `pm-raid-references` accepts an optional `sort` property with `field` (`modified-date` | `created-date`) and `direction` (`asc` | `desc`). Defaults are `field: created-date`, `direction: desc`.
...
Groups are rendered in the order determined by the `sort` config property (default: `TFile.stat.ctime` descending). When `field` is `modified-date`, `TFile.stat.mtime` is used instead.
